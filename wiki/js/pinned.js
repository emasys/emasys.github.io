...
(function ($) {
    ...
    var methods = {
        ...
        init: function (options) {
            ...
            return this.each(function () {
                ...
                $(this).data("stickyPanel.state", {
                    stickyPanelId: id,
                    isDetached: false,
                    parentContainer: $((options.parentSelector
        ...
        scroll: function (event) {
            ...
            var parentContainer = node
                .data("stickyPanel.state")
                .parentContainer;
            var parentHeight = parentContainer.height();
            var nodeHeight = node.outerHeight(true);
            var scrollTop = o.parentSelector
                ? parentContainer.scrollTop()
                : $(document).scrollTop();
            var docHeight = o.parentSelector
                ? parentContainer.height()
                : $(document).height();
            var HeightDiff = o.parentSelector
                ? parentHeight
                : (docHeight - parentHeight);

            var top = o.parentSelector
                ? node
                    .position()
                    .top
                : node
            ...
            var TopDiff = topdiff < 0
                ? 0
                : topdiff;

            // ** DEBUG ** console.log("scrollTop: " + scrollTop); console.log("height: " +
            // HeightDiff); console.log("TopDiff: " + TopDiff);
            ...
            scrollTop > TopDiff && !isDetached) {
                ...
                // MOVED: savePanelSpace before afterDetachCSSClass to handle
                // afterDetachCSSClass changing size of node savePanelSpace
                ...
                // save inline css
                node.data("Original_Inline_CSS", (!node.attr("style")
                    ? ""
                    : node.attr("style")));
        ...
        _unstick: function (nodeRef) {
            nodeRef
                .data("stickyPanel.state")
                .parentContainer
                .unbind("scroll.stickyPanel_" + nodeRef.data("stickyPanel.state").stickyPanelId);
        }
    };
